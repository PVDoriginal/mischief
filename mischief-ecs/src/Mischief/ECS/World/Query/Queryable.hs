...
instance (Queryable' qd out) => Queryable qd out where
  runQueryEntity = runQueryEntity'
  runQueryInternal = runQueryInternal'
  queryTypes = queryTypes'

-- instance (Queryable' qd out, Mappable MapQueryValidity out out') => Queryable qd out' where
--   runQueryEntity a b c = fmap (mapTuple @MapQueryValidity) <$> runQueryEntity' a b c
--   runQueryInternal a b c = map (\(a, b) -> (a, mapTuple @MapQueryValidity b)) <$> runQueryInternal' a b c
--   queryTypes = queryTypes'
...
instance (Component c, Queryable' q out) => RelQuery Inclusive (R c (Q q)) (RelQueryOutput [out]) where
  relRunQueryEntity (R (Q q)) world entity = do
    ...
    case fmap (traverse $ \r -> runQueryEntity' q world r.target) res of
      ...
      Just x -> do
        x <- x
        case x of
          [] -> pure $ Just InvalidRelQueryOutput
          x -> pure $ Just $ RelQueryOutput $ catMaybes x

  relRunQueryInternal (R (Q q)) archetypes world = do
    res <- relRunQueryInternal @Inclusive (R @c Any) archetypes world
    for res $ \(e, rels) -> do
      a <- (traverse $ \r -> runQueryEntity' q world r.target) rels

      return
        ( e,
          case a of
            [] -> InvalidRelQueryOutput
            x -> RelQueryOutput $ catMaybes x
        )

  relQueryTypes _ = Set.singleton (typeRep $ Proxy @c, RelQ)

instance (Component c, Queryable' q out) => RelQuery Exclusive (R c (Q q)) (RelQueryOutput out) where
  relRunQueryEntity (R (Q q)) world entity = do
    ...
    case fmap (\r -> runQueryEntity' q world r.target) res of
      ...
      Just x -> do
        ...
        case x of
          Nothing -> pure Nothing
          Just x -> pure $ Just $ RelQueryOutput x
  relRunQueryInternal (R (Q q)) archetypes world = do
    res <- relRunQueryInternal @Exclusive (R @c Any) archetypes world
    for res $ \(e, rels) -> do
      r <- (\r -> runQueryEntity' q world r.target) rels
      case r of
        Nothing -> return (e, InvalidRelQueryOutput)
        Just x -> return (e, RelQueryOutput x)
