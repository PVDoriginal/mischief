...
-- instance (IntoQueryFilter q) => EraseIntoStorage q (QueryFilter a) where
...
-- filterArchetype (a `QFAnd` b) w c = do
--   x <- filterArchetype a w c
--   y <- filterArchetype b w c
--   return $ x && y
-- filterArchetype (a `QFOr` b) w c = do
--   x <- filterArchetype a w c
--   y <- filterArchetype b w c
--   return $ x || y
...
-- with :: a -> QueryFilter a
-- with = undefined

-- class IntoQueryFilter qf where
--   intoQueryFilter :: qf -> QueryFilter a
...
-- checkF :: (TypeRep, Maybe Entity, Maybe Any, ErasedCheck) -> QueryFilter EntityFilter
